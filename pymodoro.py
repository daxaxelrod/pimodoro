...
class Pymodoro:
    ...
    def alert(self, *args):
        if not self.time_left.get():
            messagebox.showinfo('Timer Done', "Your timer is up!")
    ...
    def update(self):
        ...
        if self.running and time_left:
            mins, secs = self.minutes_seconds(time_left)
            self.timer_text.set(
                '{:0>2}:{:0>2}'.format(mins, secs)

            )
            self.time_left.set(time_left-1)
        else:
            mins, secs = self.minutes_seconds(DEFAULT_GAP)
            self.timer_text.set(
                '{:0>2}:{:0>2}'.format(mins, secs)
